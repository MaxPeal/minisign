...
RUN export GPGDIR=/tmp/gpgv-tmp \
  && export GPGOPS="--batch --fixed-list-mode --verbose --with-colons  --with-fingerprint --with-fingerprint --no-default-keyring --keyring ${GPGDIR}/keyring-gpgv.gpg --homedir ${GPGDIR}"; rm -fr ${GPGDIR} ; mkdir -p ${GPGDIR} ; chmod 700 ${GPGDIR} ; \
